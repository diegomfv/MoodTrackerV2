...
class SaveCommentUsecaseTest {
    ...
    private val usecase: SaveCommentUsecase = SaveCommentUsecase(repository)

    @Test
    fun `invoke when the days exists updates the comment` () {
        /* Insert model */
        ...
        runBlocking {
            usecase.invoke("newComment")
            val model = gson.fromJson(map.get("data_key"), DbContainer::class.java)
            assertEquals(model.list[0].comment, "newComment")
        }
    }

    @Test
    fun `invoke when the day does not exist creates the day with the comment` () {
        /* Insert model */
        val container = DbContainer(listOf(
        ...
        runBlocking {
            ...
            assertEquals(model.list[0].comment, "c0")
            assertEquals(model.list[1].comment, "c1")
            assertEquals(model.list[2].comment, "newComment")
        }
    }
